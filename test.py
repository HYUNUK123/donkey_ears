...
# JSON 파일 경로
file_path = './datasets/BAND_21_09.json'
...
for i in range(1, len(df)):
    if df[('ID')].iloc[i] == current_ID:
        ...
        if i < len(df) -1 and df[('ID')].iloc[i] == df[('ID')].iloc[i+1]:
            df_real = df_real._append({'Q': current_TEXT, 'A': df['TEXT'].iloc[i] + ' ' + df['TEXT'].iloc[i+1]}, ignore_index=True)
        else:
            df_real = df_real._append({'Q': current_TEXT, 'A': df['TEXT'].iloc[i]}, ignore_index=True)
        current_ID = df['ID'].iloc[i]
        current_TEXT = df['TEXT'].iloc[i]

print(df_real)

df_real.to_csv("./datasets/example.csv",index=False)